...
namespace astra
{
  ...
  void Camera::goToNextPageItem()
  {
    ...
    if (this->y == y - systemConfig.screenHeight)
      moving = false;
  }
  ...
  void Camera::goToListItemPage(uint8_t _index)
  {
    ...
    if (_index == 0)
      _page = 0;
    else if (_index % maxItemPerScreen == 0)
      _page = _index / maxItemPerScreen;
    else
      _page = floor(_index / maxItemPerScreen);
    go(0, _page * systemConfig.screenHeight);

    if (this->y == _page * systemConfig.screenHeight)
      moving = false;
  }
  ...
  void Camera::goToListItemRolling(std::vector<float> _posSelector)
  {
    ...
    moving = true;
    if (outOfView(_posSelector[0], _posSelector[1]) == 1)
      direction = 1;
    if (outOfView(_posSelector[0], _posSelector[1]) == 2)
      direction = 2;

    if (direction == 1)
    {
      go(_posSelector[0], _posSelector[1]);
      if (this->x == 0 - _posSelector[0] && this->y == 0 - _posSelector[1])
        direction = 0;
    }
    if (direction == 2)
    {
      go(_posSelector[0], _posSelector[1] + astraConfig.listLineHeight - systemConfig.screenHeight);
      if (this->x == 0 - _posSelector[0] && this->y == 0 - (_posSelector[1] + astraConfig.listLineHeight - systemConfig.screenHeight))
        direction = 0;
    }

    if (!outOfView(_posSelector[0], _posSelector[1]))
      moving = false;
  }
  ...
  void Camera::goToTileItem(uint8_t _index)
  {
    ...
    if (this->x == 0 - _index * (astraConfig.tilePicWidth + astraConfig.tilePicMargin))
      moving = false;
  }
  ...
  void Camera::update(Menu *_menu, Selector *_selector)
  {
    ...
    {
      if (astraConfig.listPageTurningMode == 0)
        goToListItemPage(_menu->selectIndex);
      else if (astraConfig.listPageTurningMode == 1)
        goToListItemRolling(_selector->getPosition());
    }
